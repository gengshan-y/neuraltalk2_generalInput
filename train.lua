--[[
 ...
 - Description:     A modified version of neural talk for video captioning.
--]]
...
cmd:option('-checkpoint_path', 'output', 'folder to save checkpoints and training histories into (empty = this folder)')
cmd:option('-language_eval', 0, 'Evaluate language as well (1 = yes, 0 = no)? BLEU/CIDEr/METEOR/ROUGE_L? requires coco-caption code from Github.')
cmd:option('-losses_log_every', 25, 'How often do we snapshot losses, for inclusion in the progress dump? (0 = disable)')
cmd:option('-verbose', true, 'if print detailed evaluation result')
...
if string.len(opt.start_from) > 0 then
  ...
  protos.cnn = net_utils.build_fc({input_size = DataLoader:getFeatDim(),
               encoding_size = opt.input_encoding_size, backend = cnn_backend})
...
local feat_cuda
if opt.gpuid >= 0 then
  feat_cuda = torch.CudaTensor()  -- declare a cuda tensor for gpu mode
else
  feat_cuda = torch.FloatTensor()  -- declare a float tensor for cpu mode
end
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    n = n + data.feat:size(1)

    -- forward the model to get loss
    feat_cuda:resize(data.feat:size()):copy(data.feat)
    local feats = protos.cnn:forward(feat_cuda)
    ...
    if verbose then
      print('\nstart evaluating validation set ...')
    end
    for k=1,#sents do
      local entry = {feat_path = data.infos[k].file_path, caption = sents[k]}
      table.insert(predictions, entry)
      if verbose then
        print(string.format('feature from %s:\n%s', data.infos[k].file_path, sents[k]))
        -- get true sentence
        print('true: ' .. data.infos[k].captions[1] .. '\n')
      end
    end
...
-- Loss function
-------------------------------------------------------------------------------
local iter = 0  -- have to move it beofre lossFun(), to get access
local function lossFun()
  ...
  -----------------------------------------------------------------------------
  -- get batch of data  
  local data = loader:getBatch{batch_size = opt.batch_size,
                               split = 'train', seq_per_img = opt.seq_per_img}
  feat_cuda:resize(data.feat:size()):copy(data.feat)
  ...
  if opt.finetune_cnn_after >= 0 and iter >= opt.finetune_cnn_after then
    ...
    local dx = protos.cnn:backward(feat_cuda, dfeats)
  end
  ...
  if opt.verbose and iter % 50 <= 0 then
    local vocab = loader:getVocab()
    local seq = protos.lm:sample(feats)
    local sents = net_utils.decode_sequence(vocab, seq)
...
while true do  
  ...
  if (iter % opt.save_checkpoint_every == 0 or iter == opt.max_iters) then
    ...
    local val_loss, val_predictions, lang_stats = eval_split('val',
                  {val_images_use = opt.val_images_use, verbose = opt.verbose})
