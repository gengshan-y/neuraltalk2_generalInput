...
local DataLoader = torch.class('DataLoader')  -- define a class DataLoader
...
function DataLoader:__init(opt)
  ...
  -- extract feature size from dataset
  local feat_size = self.h5_file:read('/feats'):dataspaceSize()
  self.num_feats = feat_size[1]
  self.num_channels = feat_size[2]
  print(string.format('read %d features of size %d', self.num_feats,
                                                           self.num_channels))
  ...
  self.split_ix = {}
  self.iterators = {}
  for i,feat_info in pairs(self.info.feats_info) do
    local split = feat_info.split
  ...
  for k,v in pairs(self.split_ix) do
    print(string.format('assigned %d features to split %s', #v, k))
  end
end
...
function DataLoader:getFeatDim()
...
function DataLoader:getBatch(opt)
  ...
  local feat_batch = torch.FloatTensor(batch_size, self.num_channels)
  ...
  for i=1, batch_size do
    ...
    -- fetch the feature from h5
    local feat = self.h5_file:read('/feats'):partial({ix,ix}, {1,self.num_channels})
    feat_batch[i] = feat
    ...
    local info_struct = {}
    info_struct.file_path = self.info.feats_info[ix].file_path
    info_struct.captions = self.info.feats_info[ix].captions
    table.insert(infos, info_struct)
  end

--[[
  -- print out decoded training sentence to check correctness 
  print('decoding training word labels for checking ...')
  sents = net_utils.decode_sequence(self.ix_to_word, label_batch:transpose(1,2))
  for k = 1, #sents do
    print(string.format('feature from %s: %s', infos[k].file_path, sents[k]))
  end
--]]

  local data = {}
  data.feat = feat_batch
